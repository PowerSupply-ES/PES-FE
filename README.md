...
`자체적인 컴파일러`를 통해 오류가 발생할 시, `컴파일 에러`인지, `테스트 케이스 불일치`에 의한 것인지 구분하여 메시지 제공</br>
...
2명의 재학생이 신입생의 풀이에 `comment` 완료 시, 풀이 결과가 나옴
</br></br>
